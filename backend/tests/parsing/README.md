...
`test_processes.py` 使用真实 Python 子进程，在 Selector 事件循环下验证：
中文及空格路径、结果与直接解析一致、业务错误传递、意外退出、超时、取消。
故障测试只在子进程内将解析函数替换为长时间等待，并通过就绪文件确认它
已经进入解析阶段；父进程的启动、轮询、终止和回收逻辑保持真实执行。
测试还检查输出临时文件已关闭，并在测试失败时回收自身启动的子进程。

Windows 本地解析测试共 18 passed，其中子进程测试 6 passed。
这不包含 Worker 被强杀后的子进程清理、OCR 下级进程、内存限制或
Linux 进程行为验证；也不替代后续 API 健康接口与 Worker 的集成测试。
